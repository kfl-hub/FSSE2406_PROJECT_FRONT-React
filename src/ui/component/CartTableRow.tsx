...
import {deleteCartItem} from "../../api/CartApi.ts";
import ShoeSizeSelectorDemo from "./ShoeSizeSelectorDemo.tsx";

type Props = {
    key: number,
    item: CartItemDto,
    onDeleteItem:(punycode:number)=>void,
}

export default function CartTableRow({key, item,onDeleteItem}: Props) {
    ...
    return (
        <TableRow key={key} sx={{backgroundColor: 'rgba(255, 255, 255, 0.85)', p: 4}}>
            <TableCell><Box
                sx={{display: "flex", alignItems: "center"}}
            ><img width={150} src={item.imageUrl} alt={"Item-image"}/>
                <Box marginLeft={4}>
                <Typography variant={"h4"}>{item.name}</Typography>
                    <ShoeSizeSelectorDemo category={"menShoes"} value={sizeValue} handleOnSizeValueChange={handleOnSizeValueChange}/>
                </Box>
            </Box></TableCell>
            ...
            <TableCell><IconButton color={"inherit"}
