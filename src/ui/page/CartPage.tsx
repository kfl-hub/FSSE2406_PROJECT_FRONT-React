...
export default function CartPage() {
  ...
  const [checkOutDisable, setCheckOutDisable] = useState<boolean>(false);
  ...
  const handleCheckOutDisable = (disable:boolean) => {
    setCheckOutDisable(disable);
  }
                ...
                <TableBody>
                  {cartItemDtoList.map((item) => (
                    <CartTableRow key={item.cid} item={item} onDeleteItem={onDeleteItem}
                                  onUpdateItem={onUpdateItem} handleCheckOutDisable={handleCheckOutDisable}/>
                  ))}
                      ...
                      <Box
                        ...
                        <Box height={56} display={"flex"} alignItems={"center"}
                          ...
                          <Typography sx={{textDecoration: 'line-through'}}
                                      variant={"h6"}>${tax.toFixed(1)}</Typography>
                        </Box>
                        <Divider variant="middle"/>
                        <Box height={56} display={"flex"} alignItems={"center"}
                             justifyContent={"space-between"}>
