...
export const patchCartItem= async (pid:number,quantity:number,sizeValue:string|number)=>{
    try{
        const response= await axios.patch<CartItemDto>(`${baseUrl}/cart/${pid}/${quantity}/${sizeValue}`
          ,null
          ,await getAuthConfig()
        )
        return response.data;
